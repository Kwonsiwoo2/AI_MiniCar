...
bool ESP8266_Begin()
{
	...
	{
		if(SendATandExpectResponse("ATE0","OK\r\n")||SendATandExpectResponse("AT","OK\r\n"))
		    return true;
	}
	return false;
}
...
uint8_t ESP8266_Send(char* Data)
{
	char _atCommand[DEFAULT_BUFFER_SIZE];
	memset(_atCommand, 0, DEFAULT_BUFFER_SIZE);
	sprintf(_atCommand, "AT+CIPSEND=%d", (strlen(Data)+2));
	_atCommand[DEFAULT_BUFFER_SIZE-1] = 0;
	// AT+CIPSEND=N 전송
	AT_Send_Blind_Command(_atCommand);
	// 조이스틱 데이터 문자열 전송
	_delay_ms(100);
	AT_Send_Blind_Command(Data);
...
void ESP8266_Print_AT_Command_Response(char* ATCommand) {
	Start_Read_Response(ATCommand);
	UART1_sendString(ATCommand);

    char buffer[DEFAULT_BUFFER_SIZE];
    uint16_t len = Read_Data(buffer);
    if (len > 0) {
        buffer[len] = '\0'; // null-terminate the string
        UART1_sendString(buffer); // 또는 UART1_sendString(buffer);
    }
}

// AT 명령어 전송 및 응답을 출력하는 함수
bool SendATCommandAndPrintResponse(char* ATCommand, char* ExpectedResponse) {
    ESP8266_Clear(); // 응답 버퍼 초기화
    if (SendATandExpectResponse(ATCommand, ExpectedResponse)) {
        ESP8266_Print_AT_Command_Response(ATCommand); // 응답을 터미널에 출력
        return true; // 명령어 성공
    }
    return false; // 명령어 실패
}
...
void ESP8266_Start_Socket_Communication() {
    ...
    sprintf(buff, "AT+CIPSEND=%d\r\n", joystickDataLen+JOYSTICK_DATA_DEFAULT_LEN);
    AT_Send_Blind_Command(buff);
}